...
export const PuzzleEditor = forwardRef<HTMLTextAreaElement, PuzzleEditorProps>(
    ({ expression, onChange, onSolve }, ref) => {
        ...
        const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
            if (e.key === 'Enter' && !e.shiftKey && validation.valid) {
                e.preventDefault();
                onSolve();
            }
        ...
        return (
            <div className="puzzle-input-container">
                ...
                <div className={`validation-msg ${expression.trim() ? 'visible' : ''} ${validation.valid ? 'success' : 'error'}`}>
                    {validation.message || 'Enter a cryptarithmetic puzzle to solve'}
                </div>
            </div>
        );
    }
...
PuzzleEditor.displayName = 'PuzzleEditor';